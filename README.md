...
#### VCD 3.0 支持
* **光盘结构与启动配置识别**：自动探测 VCD 3.0 规范光盘或目录结构，解析 `AUTORUN.CLS` 与 `ENREACH.CLS` 启动配置文件，准确定位光盘启动首页与开场配置
* **编译型复合交互页面解析**：深度解析 `<COMPHTML>` / `.CHM` 二进制页面文件，支持页面图层布局、多边形/矩形点击热区、页面导航跳转表与复合覆盖图层（Overlay）合成
* **调色板图像高效解码**：高速解码 `<YUVBMP>` / `.YBM` 8 位索引调色板图像格式并即时转换为 RGBA 画布纹理，支持高精度色度抽样与 Alpha 通道透明度合成
* **VCDSCRIPT 脚本解释与虚拟执行机**：内置完整的 VCDSCRIPT 抽象语法树与协程虚拟机（VM），原生支持变量计算、条件分支（`IF...THEN...ELSE`）、子程序调用（`GOSUB`）、伪随机数（`CALL RAND`）、按键等待（`CALL IRKEY`）与动画时序延时
* **多媒体音视频与卡拉OK集成**：无缝调度 MPEG-1 视频流回放（`.DAT`）、循环背景音乐（`BGSOUND`）与即时按键音效（`.WAV`），完整实现卡拉OK播放列表操作指令集

#### VCD 2.0 支持
* **白皮书 PBC (Playback Control) 状态机**：完整解析与执行 `LOT.VCD`（位置查找表）与 `PSD.VCD`（播放序列描述表），支持 `PlayList`、`SelectionList` 与 `EndList` 交互跳转
* **静态与动态菜单**：支持 `/SEGMENT/ITEMxxxx.DAT` 静态高解析度菜单帧解码渲染，以及标准 MPEG-1 动态交互式视频菜单流畅回放
* **多功能遥控与键盘导航**：支持多位数曲目/选单缓冲输入、超时（2.0s）自动确认、`Enter` 确认与精准物理音轨寻道；提供专属“PBC”按钮随时唤回主菜单
* **全格式自适应与模式切换**：精确区分 VCD 2.0 PBC 经典交互与 VCD 1.0 纯线性流媒体播放，支持在不同播放模式间平滑无缝切换
* **曲目与时间码索引**：精确解析 `ENTRIES.VCD` 入口点，曲目列表时间码采用清晰的 `MM:SS.FF` 格式展示

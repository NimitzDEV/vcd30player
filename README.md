...
| 功能分类 | 具体功能特性 | VCD 1.0 / 1.1 | VCD 2.0 (标准 PBC) | VCD 2.0 (扩展 PBC-X) | VCD 3.0 (互动版) | Super VCD (SVCD) |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| **光盘识别** | 目录规范与文件结构自动探测 | ✅ | ✅ | ✅ | ✅ | 🟡 (仅探测) |
| | 系统信息总表解析 (`INFO.VCD`) | ✅ | ✅ | ✅ | ✅ | 🟡 (仅头部) |
| | 播放入口点与章节索引 (`ENTRIES.VCD`) | ✅ | ✅ | ✅ | ✅ | ❌ |
| | 互动启动配置解析 (`AUTORUN.CLS`) | ➖ | ➖ | ➖ | ✅ | ➖ |
| **音视频解码** | CD-XA 扇区解复用 (Mode 2 Form 2) | ✅ | ✅ | ✅ | ✅ | ❌ |
| | MPEG-1 视频流回放 (352×288 / 352×240) | ✅ | ✅ | ✅ | ✅ | ❌ (MPEG-2) |
| | 高清静态画面解码 (`SEGMENT/ITEMxxxx.DAT`) | ➖ | ✅ | ✅ | ✅ | ❌ |
| | MPEG-1 Audio Layer II 音频流解码 | ✅ | ✅ | ✅ | ✅ | ❌ |
| | 左右声道伴奏/原唱实时切换 | ✅ | ✅ | ✅ | ✅ | ❌ |
| | 静态画面段伴音频流同步回放 | ➖ | ✅ | ✅ | ➖ | ❌ |
| | 独立音效与背景音乐 (`.WAV` / `BGSOUND`) | ➖ | ➖ | ➖ | ✅ | ➖ |
| **播放控制** | 线性音轨顺序回放与曲目导航 | ✅ | ✅ | ✅ | ✅ | ❌ |
| | 白皮书交互状态机 (`PlayList` / `SelectionList` / `EndList`) | ➖ | ✅ | ✅ | 🟡 (兼容层) | ❌ |
| | 播放时长限制 (`ptime`) 与选单重播 (`loop_count`) | ➖ | ✅ | ✅ | ➖ | ❌ |
| | 等待延时 (`wtime`) 与超时自动推进 | ➖ | ✅ | ✅ | ➖ | ❌ |
| | 遥控器数字键缓冲选歌与 2 秒超时确认 | ✅ | ✅ | ✅ | ✅ | ❌ |
| | 专属 PBC 状态与选单层级返回按键 | ➖ | ✅ | ✅ | ➖ | ❌ |
| **图形与交互** | 复合页面与图层排版 (`<COMPHTML>` / `.CHM`) | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 8 位调色板图像解码与 Alpha 混合 (`.YBM`) | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 浮动视频控制栏与交互进度条 | ✅ | ✅ | ✅ | ✅ | ❌ |
| | 覆盖页面与弹窗图层合成 (Overlay) | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 多边形与矩形热区命中检测 | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 选单按钮热区鼠标悬停手型光标 | ➖ | ➖ | ✅ | ✅ | ➖ |
| | 选单按钮热区鼠标直接点击跳转 | ➖ | ➖ | ✅ | ✅ | ➖ |
| | 选单热区高亮框可视化辅助线 | ➖ | ➖ | ✅ | ✅ | ➖ |
| | 荧光绿 CRT 风格屏幕提示 (OSD / 时间码) | ✅ | ✅ | ✅ | ✅ | ❌ |
| **脚本引擎** | VCDSCRIPT 抽象语法树与协程虚拟机 | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 变量数值运算与条件分支 (`IF...THEN...ELSE`) | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 子程序调用 (`GOSUB`) 与动画延时调度 | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 伪随机数生成 (`CALL RAND`) 与按键等待 (`CALL IRKEY`) | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 卡拉 OK 点歌列表管理与自动连续播歌 | ➖ | ➖ | ➖ | ✅ | ➖ |
| | 互动答题计分与动态数字渲染 | ➖ | ➖ | ➖ | ✅ | ➖ |

> **图例说明**：
> * ✅ **支持**：功能完全实现并通过自动化测试验证。
> * 🟡 **部分支持**：支持格式探测识别，或作为底层兼容层运行。
> * ❌ **未支持**：当前未实现（例如 SVCD 的 MPEG-2 变码率视频解码）。
> * ➖ **不适用**：该版本标准规范本身未定义此特性。

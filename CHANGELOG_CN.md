...
### 新增
- 支持扩展交互播放控制规范（PBC-X / Extended Playback Control）：
  - 支持解析与执行 `EXT/PSD_X.VCD`（扩展播放序列描述表）和 `EXT/LOT_X.VCD`（扩展位置查找表）。
  - 支持 `0x1A` 扩展选择列表（`EXT_SELECTION_LIST`）描述符解析，涵盖按钮热区数据与分支交互逻辑。
  - 智能路径优先级解析：优先加载 `EXT/` 扩展 PBC 控制数据，缺失时平滑回退至标准 `VCD/` 目录。

## [0.5.0] - 2026-09-13

### 新增
- 支持中英文语言切换，默认跟随系统语言。
- 标题栏新增设置入口，支持在弹窗中选择界面语言。
- 支持社区自定义语言包（提供 `_template.json` 模板与说明文档）。
